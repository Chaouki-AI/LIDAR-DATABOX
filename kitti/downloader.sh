...
# Used to download the KITTI dataset  (rgb images and depth maps).
# Contact: medchaoukiziara@gmail.com || me@chaouki.pro



echo "START DOWNLOADING KITTI DATASET FOR DEPTH ESTIMATION TASK"
echo "The official website of the KITTI Dataset : https://www.cvlibs.net/datasets/kitti/ "
...
# Create a semaphore with a limit of 5 concurrent jobs
MAX_JOBS=5
SEMAPHORE=$(mktemp)
mkfifo "$SEMAPHORE"
exec 9<>"$SEMAPHORE"
...
download_file() {
    ...
    if [ -f "images/$shortname" ]; then
        echo "File already exists: images/$shortname"
        return
    fi

    echo "Downloading: $shortname"
    wget -P images 'https://s3.eu-central-1.amazonaws.com/avg-kitti/raw_data/'$fullname
    unset downloading_files["$shortname"]
}

for i in ${files[@]}; do
    read -u 9
    {
        if [ -z "${downloading_files["$i"]}" ]; then
            downloading_files["$i"]=1
            download_file $i
        fi
        echo >&9
    } &
...
exec 9>&-
clear && echo "All RGB Images Downloaded Successfully, Now  Start download Depth Maps"

mkdir -p depths
wget -P depths 'https://s3.eu-central-1.amazonaws.com/avg-kitti/data_depth_annotated.zip'
clear && echo "All DATA (Images and Depth maps) Downloaded Successfully"
